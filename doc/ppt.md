...
* 在编写CSS3样式时，不同的浏览器可能需要不同的前缀。它表示该CSS属性或规则尚未成为W3C标准的一部分，是浏览器的私有属性，虽然目前较新版本的浏览器都是不需要前缀的，但为了更好的向前兼容前缀还是少不了的。 {:&.bounceIn}
...
```
* > 总结：实心圆的圆角度数最大是高度的一半；如果为实心上半圆，设定1,2位属性即可。
* 阴影效果：box-shadow
  
 > box-shadow是向盒子添加阴影。支持添加一个或者多个。

* ```css
// 支持内外阴影写法，1，2位为x，y轴必填偏移量，3位表示阴影半径，
// 4位表示颜色，默认黑色，5位表示投影方式，默认为外阴影
...
* text-overflow 与 word-wrap {:&.bounceIn}
